...
namespace Bauland.Gadgeteer
{
    ...
    public class Music
    {
        ...
        private const ushort SmReset = 0x04;
        private const ushort SmCancel = 0x10;
        private const ushort SmTests = 0x20;
        private const ushort SmSdinew = 0x800;
        private const ushort SmAdpcm = 0x1000;
        private const ushort SmLine1 = 0x4000;
        private const int SciMode = 0x00;
        private const int SciStatus = 0x01;
        private const int SciBass = 0x02;
        private const int SciClockf = 0x03;
        private const int SciWram = 0x06;
        private const int SciWramaddr = 0x07;
        private const int SciHdat0 = 0x08;
        private const int SciHdat1 = 0x09;
        private const int SciAiaddr = 0x0A;
        private const int SciVol = 0x0B;
        private const int SciAictrl0 = 0x0C;
        private const int SciAictrl1 = 0x0D;
        private const int SciAictrl2 = 0x0E;
        private const int SciAictrl3 = 0x0F;
        private const int PcmModeJointstereo = 0x00;
        private const int PcmModeDualchannel = 0x01;
        private const int PcmModeLeftchannel = 0x02;
        private const int PcmModeRightchannel = 0x03;
        private const int PcmEncAdpcm = 0x00;
        private const int PcmEncPcm = 0x04;
        ...
        private readonly byte[] _recordingBuffer;
        private readonly byte[] _recordingBuffer4Bytes;
        private bool _playing;
        private bool _recording;
        private byte _leftVolume;
        private byte _rightVolume;

        private MusicFinishedPlayingEventHandler _onMusicFinished;
        ...
        public int LeftChannelVolume
        {
            ...
            set
            {
                if (value < 0 || value > 255) throw new ArgumentOutOfRangeException(nameof(value), "value must be between 0 and 255.");

                SetVolume(value, _rightVolume);
            }
        }
        ...
        public bool IsBusy
        {
            get
            {
                lock (_playSyncRoot)
                {
                    return _playing || _recording;
                }
            }
        }

        /// <summary>Constructs a new instance.</summary>
        /// <param name="pin3">pin 3 of Socket of type S</param>
        /// <param name="spiBusId">Id string of spi bus of Socket of type S</param>
        /// <param name="pin5">pin 5 of Socket of type S</param>
        /// <param name="pin6">pin 6 of Socket of type S</param>
        public Music(int pin3, string spiBusId, int pin5, int pin6)
        {
            _playSyncRoot = new object();
            _recordSyncRoot = new object();
            _spiSyncRoot = new object();

            _commandBuffer = new byte[4];
            ...
            _sampleBuffer = new byte[] { CmdRead, SciHdat0 };
            _recordingBuffer = new byte[1024 * 8];
            _recordingBuffer4Bytes = new byte[4];

            // this.dreq = GTI.DigitalInputFactory.Create(socket, Socket.Pin.Three, GTI.GlitchFilterMode.Off, GTI.ResistorMode.PullUp, this);
            _dreq = GpioController.GetDefault().OpenPin(pin3);
        ...
        {
            if (left < 0 || left > 255) throw new ArgumentOutOfRangeException(nameof(left), "left must be between 0 and 255.");
            if (right < 0 || right > 255) throw new ArgumentOutOfRangeException(nameof(right), "right must be between 0 and 255.");

            _leftVolume = (byte)left;
            _rightVolume = (byte)right;

            CommandWrite(SciVol, (ushort)((255 - _leftVolume) << 8 | (255 - _rightVolume)));
        }
        ...
        public void RunSineTest()
        {
            ...
            CommandWrite(SciMode, SmSdinew | SmTests | SmReset);
        ...
        public void Play(byte[] data)
        {
            ...
            lock (_playSyncRoot)
            {
                if (_playing) throw new InvalidOperationException("You are arleady playing.");
                if (_recording) throw new InvalidOperationException("You cannot play while something is being recorded.");

                _playing = true;
                _playBackThread = new Thread(() => DoPlayback(data));
        ...
        public void Record(Stream stream, ushort[] patch)
        {
            ...
            {
                if (_playing) throw new InvalidOperationException("You cannot record while something is being played.");
                if (_recording) throw new InvalidOperationException("You are arleady recording.");

                _recording = true;
                _recordingThread = new Thread(() => DoRecord(stream, patch));
        ...
        {
            if (_onMusicFinished == null)
                _onMusicFinished = OnMusicFinished;
            MusicFinished?.Invoke(sender, e);
        }
        ...
        private void CommandWrite(byte register, ushort data)
        {
            lock (_spiSyncRoot)
            {
                ...
                _commandBuffer[1] = register;
                _commandBuffer[2] = (byte)(data >> 8);
                _commandBuffer[3] = (byte)data;
        ...
        private void DoPlayback(byte[] buffer)
        {
            ...
            for (int i = 0; i < size && _playing; i += 32)
            {
        ...
        private void DoRecord(Stream stream, ushort[] patch)
        {
            ...
            CommandWrite(SciClockf, 0xc000);
            CommandWrite(SciBass, 0x0000);
            CommandWrite(SciAiaddr, 0x0000);
            CommandWrite(SciWramaddr, 0xC01A);
            CommandWrite(SciWram, 0x0002);

            WritePatch(patch);

            CommandWrite(SciMode, (ushort)(CommandRead(SciMode) | SmAdpcm | SmLine1));
            CommandWrite(SciAictrl1, 0);
            CommandWrite(SciAictrl2, 4096);
            CommandWrite(SciAictrl0, 0x0000);
            ...
            while (!stop)
            {
                if (!_recording && !stopInProgress)
                {
                    CommandWrite(SciAictrl3, 0x0001);

                    stopInProgress = true;
                }

                if (stopInProgress)
                    stop = (CommandRead(SciAictrl3) & 0x0002) != 0;

                samples = CommandRead(SciHdat1);

                if (samples > 0)
                {
                    ReadData(samples);

                    stream.Write(_recordingBuffer, 0, samples << 1);
                }
            }
        ...
        private void WritePatch(ushort[] patch)
        {
            ...
            _commandBuffer[0] = 0x02;

            while (i < patch.Length)
            {
                _commandBuffer[1] = (byte)patch[i++];
                count = patch[i++];
                ...
                if ((count & 0x8000) != 0)
                {
                    count &= 0x7FFF;
                    value = patch[i++];
                    _commandBuffer[2] = (byte)(value >> 8);
                    _commandBuffer[3] = (byte)value;
        ...
        /// </summary>
        public static readonly ushort[] OggPatch =
        {
